...
{
    public struct LayerPlayer
    {
        private struct QueuedAnimation
        {
            public UAnimation state;
        ...
        public UAnimation CurrentAnimation => _queue.Current.state;
        ...
        public float Time
        {
            get
            {
                if (_queue.Current.state.IsNull()) return 0;
                return (float)_playables[_queue.Current.state].GetTime();
            }
        }
        ...
        public float NormalizedTime
        {
            get
            {
                if (!CurrentAnimation.clip) return 0;
                if (CurrentAnimation.clip.isLooping)
                {
                    return (Time / CurrentAnimation.clip.length) % 1;
                }
                else
                {
                    return Mathf.Clamp01(Time / CurrentAnimation.clip.length);
                }
            }
        }

        public LayerPlayer(PlayableGraph graph)
        {
            this = default;
        ...
        public void Queue(UAnimation animation, AnimationPlayInfo playInfo)
        {
            ...
            if (_queue.Count == 1)
            {
                CrossFade(CurrentAnimation, PlayInfo);
            }
        }
        ...
        public void CrossFade(UAnimation animation, AnimationPlayInfo playInfo)
        {
            Playable playable;

            if (!_playables.Contains(animation))
            {
                if (animation.playable == null) animation.playable = new BasicAnimationCreator(animation);
                playable = animation.playable.Create(_graph);

                _graph.Connect(playable, 0, _mixer, _playables.Count);
            ...
            playable.SetTime(0);
            playable.SetSpeed(animation.speed * playInfo.speed);
        ...
        public void Update(float deltaTime)
        {
            ...
            if (_queue.HasNext())
            {
                ...
                if (CurrentAnimation.clip.length - Time <= info.blendTime)
                {
                    _queue.Next();
                    CrossFade(CurrentAnimation, PlayInfo);
                    return;
            ...
            var playable = _playables[_queue.Current.state];

            int currentIndex = _playables.IndexOf(_queue.Current.state);
            int count = _playables.Count;

            float t = 1;
            if (!Mathf.Approximately(PlayInfo.blendTime, 0))
            {
                t = (float)playable.GetTime() / PlayInfo.blendTime;
                t = Mathf.Pow(t, 2);
            }
